...
const Courses = () => {
  ...
  const mergedCourses = [...allCourses, ...certificateCourses].sort((a, b) => a.id - b.id)
  const totalPrograms = mergedCourses.length
  const fellowshipCount = mergedCourses.filter(course => course.type === 'fellowship').length
  const pgDiplomaCount = mergedCourses.filter(course => course.type === 'pg-diploma').length
  const certificateCount = mergedCourses.filter(course => course.type === 'certificate').length
  ...
  const categories = ['all', ...new Set(mergedCourses.map(course => course.category))]
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="font-inter text-base sm:text-lg md:text-xl lg:text-2xl text-gray-100 mb-8 sm:mb-12 max-w-3xl mx-auto">
              {totalPrograms} medical programs: {fellowshipCount} Fellowships, {pgDiplomaCount} PG Diplomas & {certificateCount} Certificates
            </p>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {displayedCourses.map((course) => (
                <Link key={course.id} to={`/courses/${titleToSlug(course.title)}`} className="block bg-white rounded-3xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 overflow-hidden group cursor-pointer">
